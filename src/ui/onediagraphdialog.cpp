...
namespace {
QString fieldDescription(const video::Field &field)
{
    QStringList dimensions;
    for (const auto &axis : field.axes) {
        dimensions.append(QStringLiteral("%1 (%2 pts)").arg(axis.name).arg(axis.points.size()));
    }
    if (dimensions.isEmpty()) {
        return QObject::tr("Scalar field");
    }
    return dimensions.join(QStringLiteral(", "));
}
}

OneDimGraphDialog::OneDimGraphDialog(const std::vector<video::Project*> &projects, QWidget *parent)
    : QDialog(parent)
    , m_projects(projects)
{
    setWindowTitle(tr("New 1D graph"));
    setModal(true);
    ...
    connect(m_projectCombo, qOverload<int>(&QComboBox::currentIndexChanged), this, &OneDimGraphDialog::updateFieldsForProject);
    formLayout->addRow(tr("Project:"), m_projectCombo);

    m_fieldCombo = new QComboBox(this);
    connect(m_fieldCombo, qOverload<int>(&QComboBox::currentIndexChanged), this, &OneDimGraphDialog::updateFieldInfo);
    ...
    m_logScaleCheck = new QCheckBox(tr("Logarithmic scale"), this);
    m_gridCheck = new QCheckBox(tr("Show grid lines"), this);
    layout->addWidget(m_logScaleCheck);
    layout->addWidget(m_gridCheck);

    m_buttons = new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel, this);
    connect(m_buttons, &QDialogButtonBox::accepted, this, &OneDimGraphDialog::acceptDialog);
    connect(m_buttons, &QDialogButtonBox::rejected, this, &OneDimGraphDialog::reject);
...
void OneDimGraphDialog::updateFieldsForProject(int index)
{
    m_fieldCombo->clear();
    m_fieldInfo->clear();

    if (index < 0 || index >= m_projectCombo->count()) {
        return;
    }
    ...
    if (fieldCounter == 0) {
        m_fieldCombo->addItem(tr("No 1D-compatible fields"), -1);
...
void OneDimGraphDialog::updateFieldInfo(int index)
{
    if (index < 0) {
        m_fieldInfo->clear();
        return;
    ...
    const video::Field &field = project->fields()[fieldIndex];
    m_fieldInfo->setText(fieldDescription(field));
}

void OneDimGraphDialog::acceptDialog()
{
    if (m_fieldCombo->count() == 0 || !m_fieldCombo->isEnabled()) {
        QMessageBox::warning(this, tr("Cannot create graph"), tr("Selected project does not contain 1D fields."));
        return;
    }
    ...
    m_result.logarithmic = m_logScaleCheck->isChecked();
    m_result.gridLines = m_gridCheck->isChecked();
